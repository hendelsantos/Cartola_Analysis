    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-4">
          <StatCard icon={Target} label="Analisados" value={summary.total_players} sub="jogadores com dados" />
          <StatCard icon={CircleDollarSign} label="Subvalorizados" value={summary.undervalued_count} sub="alpha acima da média" color="text-emerald-400" />
          <StatCard icon={Sparkles} label="Gems" value={summary.gems_count} sub="C$ ≤ 12 com potencial" color="text-amber-400" />
          <StatCard icon={Trophy} label="Top Alpha" value={summary.top_alpha?.apelido || "—"} sub={summary.top_alpha ? `α ${summary.top_alpha.alpha.toFixed(2)} · ${summary.top_alpha.posicao}` : ""} color="text-green-400" />
          <StatCard icon={Zap} label="Top Momentum" value={summary.top_momentum?.apelido || "—"} sub={summary.top_momentum ? `+${summary.top_momentum.momentum.toFixed(0)}% · ${summary.top_momentum.posicao}` : ""} color="text-orange-400" />
        </div>
      )}
